...
const SuperuserDashboard = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const res = await axios.get('/api/accounts/access-requests-review/', { // Corrected endpoint as per latest backend
        headers: {
  ...
  const handleAction = async (requestId, action) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      await axios.post(`/api/accounts/access-requests-review/${requestId}/`, { action }, { // Corrected endpoint
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
