              ...
              <div className="flex items-start space-x-4">
                <div className="relative">
                  ...
                  <div 
                    className={`w-6 h-6 rounded border-2 transition-all duration-300 cursor-pointer ${
                      formData.priority_flag && isAuthenticatedUser
                        ? 'border-[#4f46e5] bg-[#4f46e5] shadow-lg shadow-[#4f46e5]/50'
                        : isAuthenticatedUser 
                          ? 'border-gray-400 hover:border-[#4f46e5]/60'
                          : 'border-gray-600 cursor-not-allowed opacity-50'
                    }`}
                    onClick={() => isAuthenticatedUser && handleChange({ target: { name: 'priority_flag', type: 'checkbox', checked: !formData.priority_flag } })}
                  >
                    {formData.priority_flag && isAuthenticatedUser && (
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    <Star className={`w-5 h-5 ${isAuthenticatedUser ? 'text-yellow-400' : 'text-gray-500'}`} />
                    <label 
                      htmlFor="priority_flag" 
                      className={`text-sm font-medium cursor-pointer ${
                        isAuthenticatedUser ? 'text-white' : 'text-gray-500'
                      }`}
                    >
                  ...
                  <p className={`text-xs ${isAuthenticatedUser ? 'text-gray-400' : 'text-gray-500'}`}>
                    {isAuthenticatedUser 
                      ? 'Priority reports receive expedited review and response times.'
                      : 'Upgrade to Premium to access priority reporting features.'
                    }
                  </p>
                </div>
              </div>
