...
// ADDED Clock to the import list
import { MessageSquare, Send, Loader2, Info, User as UserIcon, Shield, AlertCircle, Link, CheckCircle, XCircle, Clock, FileTr } from 'lucide-react';
...
const UserReportDetail = () => {
  ...
  const [isCurrentUserAdmin, setIsCurrentUserAdmin] = useState(false); // To check if current user is admin
  const [isInternalMessage, setIsInternalMessage] = useState(false); // For admin to toggle internal messages

  const messagesEndRef = useRef(null); // Ref for auto-scrolling messages

  const accessToken = localStorage.getItem('accessToken');

  // Function to decode JWT to get user role
  const getUserRole = () => {
    if (accessToken) {
      try {
  ...
  useEffect(() => {
    fetchReportDetails();
  }, [reportId, accessToken]); // Re-fetch if reportId or token changes
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Only include is_internal if the current user is an admin
      ...
      setNewMessage('');
      setIsInternalMessage(false); // Reset internal message toggle after sending
    } catch (err) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'rejected': return 'bg-red-600/30 text-red-300'; // Added rejected status color
        ...
        <div className="bg-[#10122a] p-5 rounded-lg mb-6 border border-gray-800 shadow-inner">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6 text-gray-400 text-sm border-t border-gray-700 pt-4">
            <p className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-gray-500" /> {/* Clock icon used here */}
                    ...
                    msg.is_current_user_sender
                      ...
                      : msg.is_internal && isCurrentUserAdmin // Admin's internal message (red, only seen by admins)
                        ...
                        : msg.is_sender_admin // Admin's public message (purple)
                          ...
                          : 'bg-gray-700 text-gray-100 rounded-tl-none' // Anonymous User (gray)
                  }`}
                >
                  <p className="text-xs text-gray-200 mb-1 font-semibold flex items-center gap-1">
                    {msg.is_internal ? (
                      <>
                        <Shield className="h-3 w-3 text-red-300" /> Internal Note (Admin)
                      </>
                    ) : msg.is_sender_admin ? (
                      <>
                        <Shield className="h-3 w-3 text-white" /> {msg.display_sender_name} {/* Display admin's actual name */}
                      </>
                    ) : (
                      <>
                        <UserIcon className="h-3 w-3 text-gray-300" /> {msg.display_sender_name} {/* Display "Anonymous User" */}
                      </>
                    )}
