...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const isSuperuser = decodedToken.is_superuser; // boolean

      // Store tokens
      localStorage.setItem('accessToken', access);
      localStorage.setItem('refreshToken', refresh);
      localStorage.setItem('isSuperuser', decoded.is_superuser); // This will store "true" or "false"
    ...
    setTimeout(() => {
  if (isSuperuser === true) {
    navigate('/superuser-dashboard'); // 🚀 Go to superuser dash
  } else if (userRole === 'admin') {
    navigate('/free-admin-dashboard');
  } else {
    navigate('/'); // or user dashboard
  }
}, 1500);
// Give a short delay for message to show
    } catch (error) {
