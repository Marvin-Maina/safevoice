...
} from 'lucide-react'; // Added BarChart2, Users, Flag icons
...
const FreeAdminDashboard = () => {
  ...
  const [analyticsLoading, setAnalyticsLoading] = useState(true); // New state for analytics loading
  ...
  const handleStatusUpdate = async (reportId, newStatus) => {
    ...
    try {
      ...
      fetchReports(); // Re-fetch reports to update status
      fetchAnalytics(); // Re-fetch analytics to update counts
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 sm:p-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 p-6 bg-[#10122a] rounded-lg shadow-lg border border-blue-900/20">
          <h2 className="text-2xl font-bold text-blue-300 mb-5 flex items-center gap-3">
            <BarChart2 className="h-7 w-7" /> Dashboard Overview
          </h2>
          {analyticsLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                ...
                <p className="text-3xl font-bold text-white mt-1">{analytics.total_reports}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                <p className="text-gray-400 text-sm">Pending</p>
                <p className="text-3xl font-bold text-yellow-300 mt-1">{analytics.reports_by_status.pending || 0}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                <p className="text-gray-400 text-sm">Under Review</p>
                <p className="text-3xl font-bold text-blue-300 mt-1">{analytics.reports_by_status.under_review || 0}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                <p className="text-gray-400 text-sm">Resolved</p>
                <p className="text-3xl font-bold text-green-400 mt-1">{analytics.reports_by_status.resolved || 0}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                <p className="text-gray-400 text-sm">Rejected</p>
                <p className="text-3xl font-bold text-red-400 mt-1">{analytics.reports_by_status.rejected || 0}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700">
                <p className="text-gray-400 text-sm">Escalated</p>
                <p className="text-3xl font-bold text-orange-400 mt-1">{analytics.reports_by_status.escalated || 0}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700 flex flex-col justify-center items-center">
                <Flag className="h-8 w-8 text-red-400 mb-1" />
                <p className="text-gray-400 text-sm">Priority Reports</p>
                <p className="text-3xl font-bold text-red-400 mt-1">{analytics.priority_reports_count || 0}</p>
              </div>
              <div className="bg-gray-900 p-4 rounded-md border border-gray-700 flex flex-col justify-center items-center">
                <Users className="h-8 w-8 text-purple-400 mb-1" />
                <p className="text-gray-400 text-sm">Anonymous Reports</p>
                <p className="text-3xl font-bold text-purple-400 mt-1">{analytics.anonymous_vs_identified_reports.true || 0}</p>
              </div>
            </div>
          ) : (
        ...
        <div className="mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          >
            ...
            <option value="rejected">Rejected</option> {/* Added rejected to filter options */}
          </select>
          <button
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        ...
                        report.status === 'rejected' ? 'bg-red-600/30 text-red-300' : // Added rejected status color
