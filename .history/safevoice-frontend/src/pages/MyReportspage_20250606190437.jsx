...
const fetchMyReports = useCallback(async () => {
  ...
  try {
    ...
    console.log("🔥 Actual API response shape:", response.data);

    const reportsArray = Array.isArray(response.data.results)
  ...
  // *** NEW: Fetch user-specific analytics ***
  const fetchAnalytics = useCallback(async () => {
    setLoadingAnalytics(true);
    setErrorAnalytics('');
    setAnalyticsData(null);
    try {
      // This endpoint corresponds to the 'analytics' action in ReportViewSet
      const response = await axios.get('/api/reports/user-analytics/', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
        },
      });
