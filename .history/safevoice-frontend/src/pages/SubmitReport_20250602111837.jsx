...
const SubmitReport = () => {
  ...
  // Define categories based on your models.py
  const categories = [
  ...
  useEffect(() => {
    ...
    if (accessToken) {
      try {
        ...
        // Assuming 'plan' is available in your JWT payload
  ...
  const handleSubmit = async (e) => {
    ...
    // Only append priority_flag if the user is premium and checked it
    if (isPremiumUser && formData.priority_flag) {
      data.append('priority_flag', true);
    } else {
      data.append('priority_flag', false); // Ensure it's explicitly false if not set by premium user
    }
    ...
    try {
      ...
      // navigate('/dashboard'); // Example: navigate to a dashboard after submission
    } catch (error) {
      ...
      if (error.response && error.response.data) {
        if (typeof error.response.data === 'string') {
          ...
          errorMessage = `Description: ${error.response.data.description.join(', ')}`;
        } else if (error.response.data.file_upload) {
          errorMessage = `File: ${error.response.data.file_upload.join(', ')}`;
        } else if (error.response.data.non_field_errors) {
            errorMessage = error.response.data.non_field_errors.join(', ');
        }
        else if (error.response.data.non_field_errors) { // Handle specific backend validation messages like free tier limit
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center">
            <input
              ...
              onChange={handleChange}
              disabled={!isPremiumUser} // Disable if not a premium user
