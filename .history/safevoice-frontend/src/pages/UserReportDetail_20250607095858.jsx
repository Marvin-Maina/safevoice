...
import { API_BASE_URL } from './config'; // Adjust path if config.js is elsewhere
...
const UserReportDetail = () => {
  ...
  // Storing accessToken in localStorage is generally NOT recommended due to XSS vulnerabilities.
  // For production, consider using HTTP-only cookies for better security.
  const accessToken = localStorage.getItem('accessToken');

  // Memoize getUserRole to prevent unnecessary re-runs
  ...
  }, [getUserRole]); // Dependency on memoized getUserRole
  ...
  const fetchReportDetails = useCallback(async () => {
    ...
    try {
      ...
      const messagesResponse = await axios.get(`${API_BASE_URL}/reports/${reportId}/comments/`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      setMessages(messagesResponse.data);

    } catch (err) {
      let errorMessage = 'Failed to load report details or messages.';
      if (err.response) {
        // The request was made and the server responded with a status code
        // that falls out of the range of 2xx
        if (err.response.status === 404) {
          errorMessage = 'Report not found. It might have been deleted or never existed.';
        ...
        errorMessage = 'Error setting up the request for report details.';
      }
  ...
  }, [reportId, accessToken]); // Dependencies for useCallback

  useEffect(() => {
    fetchReportDetails();
  }, [fetchReportDetails]); // Dependency on memoized fetchReportDetails
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/reports/${reportId}/comments/`, payload, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });

      setMessages((prevMessages) => [...prevMessages, response.data]);
      setNewMessage('');
      setIsInternalMessage(false); // Reset internal message checkbox after sending
    } catch (err) {
      let sendErrorMessage = 'Failed to send message. Please try again.';
      if (err.response && err.response.data && err.response.data.detail) {
        sendErrorMessage = err.response.data.detail;
      } else if (err.message) {
        sendErrorMessage = `Error: ${err.message}`;
      }
  ...
  // --- Error State ---
  if (error && !report) { // Show specific error if report couldn't be loaded at all
    return (
  ...
  // --- Report Not Found / Access Denied (after loading) ---
  if (!report && !loading) { // This case should ideally be caught by the error state if API returns 404/403
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-white flex justify-center items-center p-6">
        <div className="text-center animate-fade-in">
          ...
          {error && <p className="text-red-400 text-sm mt-2">{error}</p>} {/* Show any specific error message */}
        </div>
      </div>
    );
  }
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'resolved': return <CheckCircle className="h-4 w-4" />; // Changed to CheckCircle for resolved
      case 'escalated': return <AlertCircle className="h-4 w-4" />;
      case 'rejected': return <XCircle className="h-4 w-4" />; // Changed to XCircle for rejected
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700/50 p-6 sm:p-8 animate-scale-in">
                ...
                {report.file_upload && (
                  <div className="flex items-center gap-3 p-3 bg-slate-700/30 rounded-lg hover-scale transition-all duration-200">
                    ...
                    <div>
                      ...
                      >
                        View File <Link className="h-4 w-4" />
                      </a>
                    </div>
                  </div>
                )}
            ...
            <div className="bg-slate-800/40 backdrop-blur-sm rounded-xl mb-6 border border-slate-700/30 shadow-lg animate-fade-in">
              ...
              <div className="p-6 h-[400px] sm:h-[500px] overflow-y-auto space-y-4 custom-scrollbar"> {/* Added custom-scrollbar */}
                {messages.length === 0 ? (
                      ...
                      <div
                        className={`max-w-[75%] p-4 rounded-2xl shadow-lg transition-all duration-200 hover-scale ${
                          msg.is_current_user_sender
                            ...
                            : msg.is_internal && isCurrentUserAdmin // Only admins see internal messages as internal
                              ? 'bg-red-600/20 text-red-100 rounded-bl-md border border-red-500/30 backdrop-blur-sm'
                              : msg.is_sender_admin // Admin public message
                        ...
                        <div className="flex items-center gap-2 mb-2 text-xs opacity-80">
                          {msg.is_internal ? (
                            ...
                            <>
                              ...
                              <span className="font-semibold">{msg.display_sender_name} (Admin)</span>
                            </>
                          ) : (
                            <>
            ...
            <form onSubmit={handleSendMessage} className="space-y-4 animate-fade-in">
              {error && ( // Display error if message sending fails
