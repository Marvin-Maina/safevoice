...
print(f"DEBUG: Loading reports/views.py from: {__file__}")
...
# These should NOT include AdminReportSerializer, AdminUserSerializer, or ReportAnalyticsSerializer
...
# >>>>>>>>>>>>>>>>>> THIS IS THE CRITICAL IMPORT BLOCK <<<<<<<<<<<<<<<<<<<<
# Imports from the adminpanel app's serializers
...
class AdminAnalyticsView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Reports by status (formatted as a dictionary)
        ...
        # Reports by category (formatted as a dictionary)
        ...
        # Reports by month for the last 6 months
        today = timezone.now()
        months_data = []
        for i in range(6):
            # Calculate the start of the month (e.g., June 1st, May 1st, etc.)
            # Using timedelta for days can be tricky with month boundaries,
            # so a more robust way is to adjust month and year.
            # For simplicity, sticking to days=30*i for now, but be aware of edge cases.
            month_start = (today - timezone.timedelta(days=30*i)).replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            
            # Calculate the end of the month (last microsecond of the current month)
            # This is tricky with replace, better to calculate next month's start and subtract
        ...
        months_data.reverse() # Show newest month last

        # Premium vs Free Reports
        premium_vs_free_reports = {}
        # Ensure 'is_premium_report' field exists on your Report model for this to work
        if hasattr(Report, 'is_premium_report'):
            premium_vs_free_query = Report.objects.values('is_premium_report').annotate(count=Count('is_premium_report')).order_by('is_premium_report')
            premium_vs_free_reports = {bool(item['is_premium_report']): item['count'] for item in premium_vs_free_query}
        else:
            # Fallback if is_premium_report doesn't exist (e.g., all reports are considered 'free' or default)
            premium_vs_free_reports = {True: 0, False: total_reports} # Assume all are free if no premium field

        # Anonymous vs Identified Reports
        anonymous_vs_identified_query = Report.objects.values('is_anonymous').annotate(count=Count('is_anonymous')).order_by('is_anonymous')
        anonymous_vs_identified_reports = {bool(item['is_anonymous']): item['count'] for item in anonymous_vs_identified_query}
        ...
        data = {
            ...
            'reports_by_category': reports_by_category, # Now a dictionary
            'reports_by_status': reports_by_status,     # Now a dictionary
