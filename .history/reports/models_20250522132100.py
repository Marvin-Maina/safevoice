...
def clean_filename(filename):
    name, ext = os.path.splitext(filename)  # fixed typo here
...
def user_report_path(instance, filename):
    ...
    return f"reports/{user_id}/{uuid.uuid4()}_{clean_name}"  # use sanitized filename
...
class Report(models.Model):
    ...
    file_upload = models.FileField(
        ...
        blank=True,
        null=True,
        validators=[validate_upload_file]  # fixed typo
    )
