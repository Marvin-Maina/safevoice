...
from .serializers import (
    ...
    AdminUserSerializer, # Assuming you have this serializer for UserListView/UserUpdateView
)
...
import csv # Added for ExportReportsView

# Define your custom permission classes if they are in the same file or import them
# For example:
class IsAdminOrPremiumAdmin(permissions.BasePermission):
    def has_permission(self, request, view):
        return request.user and request.user.is_authenticated and (request.user.role == 'admin' or request.user.plan == 'premium')

class IsPremiumAdmin(permissions.BasePermission):
    def has_permission(self, request, view):
        return request.user and request.user.is_authenticated and request.user.plan == 'premium'
...
class AdminAccessRequestView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if AdminAccessRequest.objects.filter(user=self.request.user, status='pending').exists():
            raise ValidationError({"error": "You already have a pending admin access request."}) # Corrected status.HTTP_400_BAD_REQUEST to ValidationError
...
class AdminAccessRequestReviewView(APIView):
    ...
    def post(self, request, request_id):
        ...
        if action == 'approve':
            ...
            user = admin_request.user # Get user for notification
            # Optionally notify the user
            Notification.objects.create(
                user=user,
...
class ReportCommentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_admin():
            return ReportComment.objects.filter(report=report).order_by('sent_at')
        else:
            # Only show comments if the user is the submitter AND it's not an internal comment
            return ReportComment.objects.filter(report=report, report__submitted_by=self.request.user, is_internal=False).order_by('sent_at')
    ...
    def perform_create(self, serializer):
        ...
        if is_internal_from_request and not self.request.user.is_admin():
...
class AdminAnalyticsView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        months_data = []
        for i in range(6):
            # Calculate month_start and month_end for the current iteration
            month_start = (today - timedelta(days=30*i)).replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            # Ensure month_end correctly captures the end of the month
        ...
        # Check if 'is_premium' exists on the Report model (this might be a custom property or field)
        # If 'is_premium' is a BooleanField, the direct values() call works.
        # If it's derived, you might need a different aggregation strategy.
        # Assuming 'is_premium' is a field or can be directly queried
...
# This class should be combined with AdminAnalyticsView or removed if redundant
# Assuming AdminAnalyticsView is the desired single source for analytics
# If you intend to have two separate analytics views (one for generic admin and one for more specific),
# you should rename ReportAnalyticsView to something distinct like PublicReportAnalyticsView if it's less restricted
# or if it's intended for a different purpose/audience.
# For now, I'm assuming AdminAnalyticsView is the primary and this is a duplicate or needs refactoring.
# I will keep the combined logic in AdminAnalyticsView and remove this second ReportAnalyticsView to avoid conflict.
