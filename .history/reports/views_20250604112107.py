...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Admins see all reports, regular users only their own
    ...
    def perform_create(self, serializer):
        ...
        # Set is_premium based on user's current plan
        serializer.save(submitted_by=user, is_premium=user.is_premium())

    def perform_update(self, serializer):
        report = self.get_object()
        user = self.request.user

        # Prevent non-admins from changing status, priority_flag, internal_notes, resolution_notes
        if not user.is_admin():
            if 'status' in serializer.validated_data and serializer.validated_data['status'] != report.status:
                raise ValidationError("You do not have permission to change report status.")
            if 'priority_flag' in serializer.validated_data and serializer.validated_data['priority_flag'] != report.priority_flag:
                raise ValidationError("You do not have permission to change priority flag.")
            if 'internal_notes' in serializer.validated_data:
                raise ValidationError("You do not have permission to add internal notes.")
            if 'resolution_notes' in serializer.validated_data:
                raise ValidationError("You do not have permission to add resolution notes.")
            
            # Allow users to update their own non-admin fields if report is still pending
            if report.status != 'pending':
                 raise ValidationError("Reports can only be updated when in 'pending' status.")


        # If admin changes status, update reviewed_by and internal_notes (if provided)
        if user.is_admin():
            if 'status' in serializer.validated_data and serializer.validated_data['status'] != report.status:
                report.reviewed_by = user
                # Automatically set resolution_notes if status becomes 'resolved'
                if serializer.validated_data['status'] == 'resolved' and 'resolution_notes' not in serializer.validated_data:
                    serializer.validated_data['resolution_notes'] = f"Resolved by {user.username} on {timezone.now().strftime('%Y-%m-%d')}."
                ...
                if report.submitted_by:
                    Notification.objects.create(
                        ...
                        report=report,
                        message=f"The status of your report '{report.title}' has been updated to '{serializer.validated_data['status']}'."
                    )
