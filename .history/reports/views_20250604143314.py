...
class AdminAnalyticsView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        print(f"--- Debugging AdminAnalyticsView (get) ---")
        ...
        if request.user.is_authenticated:
            ...
            # This block should ideally not be reached if other admin endpoints work.
        ...
        print(f"Data prepared for serializer: {data}") # <<< NEW PRINT

        serializer = ReportAnalyticsSerializer(data=data)

        if not serializer.is_valid(): # <<< MODIFIED LOGIC TO CATCH ERRORS
            print(f"Serializer validation FAILED!") # <<< NEW PRINT
            print(f"Serializer errors: {serializer.errors}") # <<< NEW PRINT
            # You might want to return a 400 Bad Request here for debugging,
            # but for now, let's just print and see if it still leads to 403.
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST) # Explicitly return 400 for debugging

        print(f"Serializer validation PASSED.") # <<< NEW PRINT
        return Response(serializer.data)
