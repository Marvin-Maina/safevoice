...
class ReportCommentSerializer(serializers.ModelSerializer):
    # This will now conditionally return the username or "Anonymous User"
    ...
    is_internal = serializers.BooleanField(read_only=True) # is_internal is set by the view, not client

    class Meta:
        model = ReportComment
        fields = [
            'id', 'report', 'sender', 'message', 'sent_at',
            'is_internal', 'is_current_user_sender', 'is_sender_admin',
            'display_sender_name' # New field for frontend display
        ]
        read_only_fields = ['id', 'report', 'sender', 'sent_at', 'is_internal']


    def get_display_sender_name(self, obj):
        # Assuming you have an is_admin() method on your User model
        if obj.sender.is_admin():
            return obj.sender.username
        # If not an admin, return a generic anonymous label
